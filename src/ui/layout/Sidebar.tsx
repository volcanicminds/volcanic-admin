 ...
 * Groups are ordered; resources fall under their `group`. Ungrouped resources
 * collect under a synthetic trailing section.
 */
...
export function Sidebar() {
  ...
  const renderItem = (res: (typeof model.resources)[number]) => (
    ...
    >
      <Icon name={res.spec.icon} className="h-4 w-4" />
      {t(res.spec.label.plural)}
    </NavLink>
  )
  ...
  return (
    <aside className="flex h-full w-60 flex-col border-r bg-card">
      <div className="flex h-14 items-center gap-2 border-b px-4">
        ...
        <span className="font-semibold">Volcanic Admin</span>
      </div>
      <nav className="flex-1 space-y-6 overflow-y-auto p-3">
        ...
        {ungrouped.length > 0 && <div className="space-y-1">{sortRes(ungrouped).map(renderItem)}</div>}
      </nav>
    </aside>
  )
}
