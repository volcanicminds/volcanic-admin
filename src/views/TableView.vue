	...
	methods: {
		initialize: async function () {
			...
			if (tableStoreConfiguration) {
				...
				const { pagination: menuPagination = {}, sorting: menuSorting = {}, filters: menuFilters = [] } = menuItem
				const modelPagination = modelConfiguration?.table?.pagination || {}
				this.params = {
					pagination: {
			...
			const storedColumnDefinitions = tableStore.tables.columnDefinitions[menuItem.name || indexSource]
		...
		updateRouteQuery: async function () {
			const [pagination, sorting, filters] = await Promise.all([
				(() => {
					return {
